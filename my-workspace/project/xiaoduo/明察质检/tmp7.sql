            ...
            FROM
              (SELECT `day`,
               ...
               WHERE `day` BETWEEN toYYYYMMDD(subtractDays(toDate('2023-03-19'), 2 - 1)) AND toYYYYMMDD(toDate('2023-03-19'))
                 ...
                 --
                 AND buyer_nick = 'one_id_263935066'
                 -- 顾客仅有一句话的不算询单（剔除单口相声）
    ...
    and `cycle` = 2
     ...
     AND replaceAll(buyer_nick,'cntaobao','') NOT IN
       (SELECT DISTINCT buyer_nick
        FROM ft_dwd.order_detail_all
        WHERE `day`BETWEEN toYYYYMMDD(subtractDays(subtractDays(toDate('2023-03-19'),2),180)) AND toYYYYMMDD(subtractDays(toDate('2023-03-19'),2))
