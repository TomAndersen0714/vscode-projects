...
-- 下拉订单状态
AND ('{{ order_status }}'=''
     OR recent_order_status IN splitByChar(',',replaceAll('{{ order_status }}', 'unorder', '')))
           AND ('{{ round_count }}'=''
      ...
      FROM
        (SELECT question_id,
         ...
         FROM
           (SELECT *
            FROM dws.voc_goods_question_stat_all
         WHERE day BETWEEN toYYYYMMDD(subtractDays(toDate('{{ day.start=week_ago }}'),dateDiff('day', toDate('{{ day.start=week_ago }}'),toDate('{{ day.end=yesterday }}'))+1)) AND toYYYYMMDD(subtractDays(toDate('{{ day.start=week_ago }}'),1))
           AND platform = 'tb' -- 下拉店铺
AND ('{{ shop_ids }}'=''
     OR shop_id IN splitByChar(',','{{ shop_ids }}') )-- 下拉订单状态
-- 下拉订单状态
