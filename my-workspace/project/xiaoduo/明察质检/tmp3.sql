    ...
    and `cycle` = {{cycle}}
     ...
     AND replaceAll(buyer_nick,'cnjd','') NOT IN
       (SELECT DISTINCT buyer_nick
        FROM ft_dwd.order_detail_all
        WHERE `day`BETWEEN toYYYYMMDD(subtractDays(subtractDays(toDate('{{ds}}'),{{cycle}}),180)) AND toYYYYMMDD(subtractDays(toDate('{{ds}}'),{{cycle}}))
...
LEFT JOIN
  (SELECT *
   ...
   WHERE `day` BETWEEN toYYYYMMDD(subtractMonths(subtractDays(toDate('{{ds}}'), {{cycle}} - 1),2)) AND toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1))
     AND shop_id = '{{shop_id}}'
     AND `cycle` = {{cycle}} and step_trade_status = 'FRONT_PAID_FINAL_NOPAID'
     ) AS persell_info
   USING(shop_id, order_id, goods_id)
WHERE if(order_info.session_id = '',persell_info.session_id,order_info.session_id) != '';
