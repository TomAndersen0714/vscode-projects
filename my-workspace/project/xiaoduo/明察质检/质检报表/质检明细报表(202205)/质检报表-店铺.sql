...
SELECT
    CASE
        ...
        ELSE platform
    END AS `平台`,
    seller_nick AS `店铺`, department_id, department_name AS `子账号分组`,
    
    count(distinct snick) AS `客服人数`,
    sum(dialog_cnt) AS `总会话量`,
    round((`总会话量`*100 + sum(add_score_sum) - sum(subtract_score_sum))/`总会话量`,2) AS `平均分`,
    -- 质检结果总览-AI质检
    `总会话量` AS `AI质检量`,
    sum(ai_subtract_score_dialog_cnt) AS `AI异常会话量`,
    sum(ai_subtract_score_sum) AS `AI扣分分值`,
    concat(toString(round((`AI异常会话量` * 100 / `总会话量`), 2)),'%') AS `AI扣分会话比例`,
    sum(ai_add_score_dialog_cnt) AS `AI加分会话量`,
    sum(ai_add_score_sum) AS `AI加分分值`,
    concat(toString(round((`AI加分会话量` * 100 / `总会话量`), 2)),'%') AS `AI加分会话比例`,
    -- 质检结果总览-人工质检
    round((0.9604 * `总会话量`) /(0.0025 * `总会话量` + 0.9604), 0) as `建议抽检量`,
    sum(manual_marked_dialog_cnt) AS `人工抽检量`,
    concat(toString(round((`人工抽检量` * 100 / `总会话量`), 2)),'%') as `抽检比例`,

    sum(manual_subtract_score_dialog_cnt) AS `人工扣分会话量`,
    sum(manual_subtract_score_sum) AS `人工扣分分值`,
    concat(toString(round((`人工扣分会话量` * 100 / `总会话量`), 2)),'%') AS `人工扣分会话比例`,
    sum(manual_add_score_dialog_cnt) `人工加分会话量`,
    sum(manual_add_score_sum) AS `人工加分分值`,
    concat(toString(round((`人工加分会话量` * 100 / `总会话量`), 2)),'%') AS `人工加分会话比例`,
    -- 质检结果总览-自定义质检
    sum(custom_subtract_score_dialog_cnt) AS `自定义扣分会话量`,
    sum(custom_subtract_score_sum) AS `自定义扣分分值`,
    concat(toString(round((`自定义扣分会话量` * 100 / `总会话量`), 2)),'%') AS `自定义扣分会话比例`,
    sum(custom_add_score_dialog_cnt) AS `自定义加分会话量`,
    sum(custom_add_score_sum) AS `自定义加分分值`,
    concat(toString(round((`自定义加分会话量` * 100 / `总会话量`), 2)),'%') AS `自定义加分会话比例`,

    -- AI质检结果
    sumMap(ai_tag_names, ai_tag_cnts) AS ai_tag_kvs,
    arrayStringConcat(arrayMap(x->toString(x),ai_tag_kvs.1),'$$') AS `AI质检标签`,
    arrayStringConcat(arrayMap(x->toString(x),ai_tag_kvs.2),'$$') AS `AI质检触发次数`,

    -- 人工质检结果
    sumMap(manual_tag_names, manual_tag_cnts) AS manual_tag_kvs,
    arrayStringConcat(arrayMap(x->toString(x),manual_tag_kvs.1),'$$') AS `人工质检标签`,
    arrayStringConcat(arrayMap(x->toString(x),manual_tag_kvs.2),'$$') AS `人工质检触发次数`,

    -- 自定义质检结果
    sumMap(custom_tag_names, custom_tag_cnts) AS custom_tag_kvs,
    arrayStringConcat(arrayMap(x->toString(x),custom_tag_kvs.1),'$$') AS `自定义质检标签`,
    arrayStringConcat(arrayMap(x->toString(x),custom_tag_kvs.2),'$$') AS `自定义质检触发次数`
