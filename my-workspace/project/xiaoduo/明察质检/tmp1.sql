            ...
            FROM
              (SELECT `day`,
               ...
               WHERE `day` BETWEEN toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1)) AND toYYYYMMDD(toDate('{{ds}}'))
                 AND shop_id = '{{shop_id}}'
                 AND session_recv_cnt > 1 -- 顾客仅有一句话的不算询单（剔除单口相声）
                 AND has_transfer IN (0,
                                      1) -- 会话中有被转接记录不算询单：转出的不算询单，转入的算一个询单
                 AND concat(shop_id,'-',cnick) NOT IN
                   (SELECT concat(shop_id,'-',cnick) -- 剔除广告、小二、内部沟通、测试账号（导入的方式剔除）
...
FROM
  (SELECT order_info.*,new_modified as modified,
          arrayMap(x-> toDateTime(modified) - toDateTime(toDateTime64(x,6)),session_start_time_arr) AS time_subtract, -- 订单时间 - 会话开始时间
          arrayFilter(x-> x>=0,time_subtract) AS time_subtract_filter, -- 为负数 则说明订单在会话前发生， 剔除
          arrayReduce('min',time_subtract_filter) AS time_subtract_min,-- 在先聊天在下单的数据中，取下单时间距离会话最近的一条 匹配
      ...
      FROM
        (SELECT `day`,
         ...
         where `day` BETWEEN toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1)) AND toYYYYMMDD(toDate('{{ds}}'))
                AND shop_id = '{{shop_id}}'
                and `cycle` = {{cycle}}
                and  concat(shop_id,'-',buyer_nick) IN -- 只看询单周期内下过单的买家
   ...
   LEFT JOIN
     (SELECT shop_id,
      ...
      WHERE `day` BETWEEN toYYYYMMDD(subtractDays(toDate('{{ds}}'),{{cycle}} - 1)) AND toYYYYMMDD(toDate('{{ds}}'))
        ...
        ) AS `order_info` USING(shop_id,
                                                       buyer_nick)
   HAVING length(time_subtract_filter) > 0) -- 为负数 则说明订单在会话前发生， 剔除
GROUP BY `day`,
         shop_id,
         session_id,
         buyer_nick,
    ...
    and `cycle` = {{cycle}}
     ...
     AND replaceAll(buyer_nick,'cnjd','') NOT IN
       (SELECT DISTINCT buyer_nick
        FROM ft_dwd.order_detail_all
        WHERE `day`BETWEEN toYYYYMMDD(subtractDays(subtractDays(toDate('{{ds}}'),{{cycle}} - 1),180)) AND toYYYYMMDD(subtractDays(toDate('{{ds}}'),{{cycle}} - 1))
          AND shop_id = '{{shop_id}}' and buyer_nick != '')
      and buyer_nick != '';
...
SELECT `day`,
      ...
      goods_id,
      if(has(groupArray(status),'created'),toString(groupArray(modified)[indexOf(groupArray(status),'created')]),'') AS created_time,
      if(has(groupArray(status),'deposited'),toString(groupArray(modified)[indexOf(groupArray(status),'deposited')]),'') AS deposited_time,
      if(has(groupArray(status),'deposited'),groupArray(order_payment)[indexOf(groupArray(status),'deposited')], if(has(groupArray(status),'created'),groupArray(order_payment)[indexOf(groupArray(status),'created')],0)) AS order_payment,
      if(has(groupArray(status),'deposited'),groupArray(goods_payment)[indexOf(groupArray(status),'deposited')], if(has(groupArray(status),'created'),groupArray(goods_payment)[indexOf(groupArray(status),'created')],0)) AS goods_payment,
      Max(step_trade_status),
      Max(step_paid_fee),
      Max(new_order_type),
      Max(goods_num),
...
SELECT order_info.`day`,
       ...
       buyer_nick,
       real_buyer_nick,
       if(persell_info.snick!='',persell_info.snick,order_info.snick) as snick,
       if(persell_info.snick!='',persell_info.session_id,order_info.session_id) as session_id,
       if(persell_info.snick!='',persell_info.session_start_time,order_info.session_start_time) as session_start_time,
       if(persell_info.snick!='',persell_info.session_end_time,order_info.session_end_time) as session_end_time,
       if(persell_info.snick!='',persell_info.is_start_by_cnick,order_info.is_start_by_cnick) as is_start_by_cnick,
       if(persell_info.snick!='',persell_info.is_end_by_cnick,order_info.is_end_by_cnick) as is_end_by_cnick,
       if(persell_info.snick!='',persell_info.focus_goods_ids,order_info.focus_goods_ids) as focus_goods_ids,
       if(persell_info.snick!='',persell_info.c_active_send_goods_ids,order_info.c_active_send_goods_ids) as c_active_send_goods_ids,
       if(persell_info.snick!='',persell_info.s_active_send_goods_ids,order_info.s_active_send_goods_ids) as s_active_send_goods_ids,
       order_id,
       goods_id,
       created_time,
       order_info.paid_time,
       if(persell_info.snick!='',persell_info.order_payment,order_info.order_payment) as order_payment,
       if(persell_info.snick!='',persell_info.goods_payment,order_info.goods_payment) as goods_payment,
       ...
       if(persell_info.snick!='',persell_info.is_refund,order_info.is_refund) as is_refund,
       1 as is_transf,
       `cycle`,
       1
FROM
  (SELECT `day`,
          ...
          0 AS is_refund,
          if(paid_time != '',1,0) AS is_transf, {{cycle}} AS `cycle`
   FROM
     (SELECT order_info.*,
 ...
 s_active_send_goods_ids_arr[index_v] AS s_active_send_goods_ids
      FROM
        (SELECT `day`,
                shop_id,
                order_id,
                toDateTime(splitByString('.',arrayReduce('min',groupArray(modified)))[1]) AS new_modified,
                argMin(buyer_nick,modified) AS buyer_nick,
        ...
        (SELECT shop_id,
         ...
         FROM
           (SELECT `day`,
                   ...
                   s_active_send_goods_ids
            FROM tmp.session_filter_all
            WHERE `day` BETWEEN toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1 + {{cycle}} - 1)) AND toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1))
              AND shop_id = '{{shop_id}}'
              AND `cycle` = {{cycle}}
              AND concat(shop_id,'-',buyer_nick) IN -- 只看询单周期内下过单的买家
                ...
                (SELECT DISTINCT concat(shop_id,'-',buyer_nick)
                 ...
                 WHERE `day` = toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1))
                   ...
                   AND order_type = 'step') )
         GROUP BY shop_id,
                  buyer_nick) AS session_1 USING(shop_id,
                                                 buyer_nick) )-- 为负数 则说明订单在会话前发生， 剔除
GROUP BY `day`,
         shop_id,
         session_id,
         buyer_nick,
...
LEFT JOIN
  (SELECT *
   FROM ft_dwd.persell_ask_order_cov_detail_all
   WHERE `day` BETWEEN toYYYYMMDD(subtractMonths(subtractDays(toDate('{{ds}}'), {{cycle}} - 1),2)) AND toYYYYMMDD(subtractDays(toDate('{{ds}}'), {{cycle}} - 1))
     AND shop_id = '{{shop_id}}'
     AND `cycle` = {{cycle}} and step_trade_status = 'FRONT_PAID_FINAL_NOPAID') AS persell_info USING(shop_id,
                                                    order_id,
                                                    goods_id)
having if(order_info.session_id = '',persell_info.session_id,order_info.session_id) != '';
