...
SELECT
    ...
    valid_dialog_cnt AS `总会话量(剔除单口相声)`,
    IF(
        valid_dialog_cnt != 0,
        round((valid_dialog_cnt*100 + valid_add_score_sum- valid_subtract_score_sum)/valid_dialog_cnt, 2),
        0.00
    ) AS `平均分(剔除单口相声)`,
...
FROM (
    ...
    FROM (
        ...
        SELECT
            ...
            -- 分值统计-总计-剔除无效会话
            sum(valid_subtract_score_sum) AS valid_subtract_score_sum,
            sum(valid_add_score_sum) AS valid_add_score_sum,
            ...
            -- 会话量统计-总计-剔除无效会话
            sum(valid_dialog_cnt) AS valid_dialog_cnt,
            -- 会话量统计-AI质检
    ...
    GLOBAL FULL OUTER JOIN (
        ...
        SELECT
            ...
            -- 自定义质检
            groupArrayIf(
                tag_name, tag_type IN ['custom_subtract', 'custom_add', 'custom_message', 'custom_dialog', 'custom']
            ) AS custom_tag_names,
            groupArrayIf(
                tag_cnt_sum, tag_type IN ['custom_subtract', 'custom_add', 'custom_message', 'custom_dialog', 'custom']
            ) AS custom_tag_cnts
        FROM (
            SELECT
