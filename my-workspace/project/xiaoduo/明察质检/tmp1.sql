...
FROM (
    ...
    WHERE day = toYYYYMMDD(toDate('{{date_time}}'))
    AND shop_id = '{{ shop_id }}'
    -- 筛选接收消息
    AND act = 'recv_msg'
    -- 筛选命中精准意图的消息
    AND precise_intent_id !=''
    -- 筛选命中指定精准意图的消息-本句核心词类型
    AND precise_intent_id GLOBAL IN (
        SELECT
            _id AS 
        FROM dim.shop_precise_intent_all
        WHERE shop_id = '{{ shop_id }}'
        AND intent_type IN ['current_keyword', '']
    )
    -- 筛选指定商品
