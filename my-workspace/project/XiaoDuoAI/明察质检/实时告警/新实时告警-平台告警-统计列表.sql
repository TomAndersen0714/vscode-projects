...
SELECT
    ...
    seller_nick AS `店铺`,
    day AS `日期`,
    platform_shop_warning_daily_cnt AS `告警总量`, -- 指定平台各个店铺当天内的告警总量
    platform_shop_warning_cnt_daily_avg AS `日均告警量`, -- 指定平台指定时间段内各个店铺的日均告警量
...
FROM (
    ...
    SELECT
        ...
        if(interval>0, round(count(1)/interval,2), 0.00) AS platform_shop_warning_cnt_daily_avg
        -- 指定平台指定时间段内各个店铺的日均告警量
    FROM (
        SELECT DISTINCT
            day, platform, seller_nick, id
    ...
    GROUP BY day,seller_nick,platform
)
GLOBAL JOIN(
    -- "平台-店铺-天"维度聚合
    SELECT
        ...
        seller_nick,
        day,
        sum(warning_type!='') AS platform_shop_warning_daily_cnt,
    ...
    FROM (
        SELECT DISTINCT
            day, platform, seller_nick, id, warning_type
        FROM xqc_ods.alert_all
        WHERE day BETWEEN toYYYYMMDD(toDate('{{ day.start=week_ago }}')) 
            AND toYYYYMMDD(toDate('{{ day.end=today }}'))
        -- 已订阅店铺
...
USING (platform, seller_nick, day)
ORDER BY day DESC, platform DESC, seller_nick ASC