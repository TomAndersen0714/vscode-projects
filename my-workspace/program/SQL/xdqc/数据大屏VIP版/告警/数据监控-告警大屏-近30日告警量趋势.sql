-- 数据监控-告警大屏-近30日告警量趋势
WITH
(SELECT toYYYYMMDD(today())) AS today,
(SELECT toYYYYMMDD(today()-30)) AS 30_days_ago,
(SELECT toYYYYMMDD(today()-60)) AS 60_days_ago
SELECT
    near_30_day_alert_stat.day AS `日期`,
    near_30_day_alert_stat.alert_cnt AS `告警量`,
    last_30_day_alert_stat.alert_cnt AS `上月同期`
FROM (
    SELECT
        day,
        date,
        alert_cnt
    FROM (
        SELECT
            day,
            count(1) AS alert_cnt
        FROM xqc_ods.alert_all FINAL
        WHERE day > 30_days_ago AND day <= today
        AND if('{{ shop_name=全部 }}'!='全部',seller_nick = '{{ shop_name=全部 }}', seller_nick!='')
        -- 已订阅店铺
        AND shop_id GLOBAL IN (
            SELECT tenant_id AS shop_id
            FROM xqc_dim.company_tenant
            WHERE company_id = '{{ company_id=61602afd297bb79b69c06118 }}'
            AND platform = '{{ platform=tb }}'
        )
        GROUP BY day
    ) AS alert_stat
    GLOBAL RIGHT JOIN (
        -- 固定时间轴
        SELECT day, date
        FROM (
            SELECT
                arrayMap(
                        x->toYYYYMMDD(toDate(x)),
                        range(toUInt32(today()-29), toUInt32(today()+1), 1)
                ) AS day,
                arrayMap(
                        x->toDate(x),
                        range(toUInt32(today()-29), toUInt32(today()+1), 1)
                ) AS date
        )
        ARRAY JOIN
            day, date
    )
    USING day
) AS near_30_day_alert_stat
GLOBAL LEFT JOIN (
    SELECT
        day,
        date,
        alert_cnt
    FROM (
        SELECT
            day,
            count(1) AS alert_cnt
        FROM xqc_ods.alert_all FINAL
        WHERE day > 60_days_ago AND day <= 30_days_ago
        AND if('{{ shop_name=全部 }}'!='全部',seller_nick = '{{ shop_name=全部 }}', seller_nick!='')
        -- 已订阅店铺
        AND shop_id GLOBAL IN (
            SELECT tenant_id AS shop_id
            FROM xqc_dim.company_tenant
            WHERE company_id = '{{ company_id=61602afd297bb79b69c06118 }}'
            AND platform = '{{ platform=tb }}'
        )
        GROUP BY day
    ) AS alert_stat
    GLOBAL RIGHT JOIN (
        -- 固定时间轴
        SELECT day, date
        FROM (
            SELECT
                arrayMap(
                        x->toYYYYMMDD(toDate(x)),
                        range(toUInt32(today()-59), toUInt32(today()-29), 1)
                ) AS day,
                arrayMap(
                        x->toDate(x),
                        range(toUInt32(today()-59), toUInt32(today()-29), 1)
                ) AS date
        )
        ARRAY JOIN
            day, date
    )
    USING day
) AS last_30_day_alert_stat
ON near_30_day_alert_stat.date - 30 = last_30_day_alert_stat.date
ORDER BY day ASC
