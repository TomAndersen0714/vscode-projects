...
FROM (
    -- 告警量
    SELECT day, shop_id, '告警总量' AS `type`, COUNT(DISTINCT id) AS cnt
    FROM xqc_ods.alert_all FIANL
    WHERE shop_id IN (
        -- 已订阅店铺
        SELECT tenant_id AS shop_id
    ...
    -- 告警自动发送提醒量
    SELECT day, shop_id, '告警提醒总量' AS `type`, COUNT(DISTINCT id)  AS cnt
    FROM xqc_ods.alert_remind_all
    WHERE shop_id IN (
        -- 已订阅店铺
        SELECT tenant_id AS shop_id
    ...
    -- 告警完结量
    SELECT day, shop_id, '告警完结总量' AS `type`, SUM(is_finished = 'True') AS cnt
    FROM xqc_ods.alert_all FIANL
    WHERE shop_id IN (
        -- 已订阅店铺
        SELECT tenant_id AS shop_id
