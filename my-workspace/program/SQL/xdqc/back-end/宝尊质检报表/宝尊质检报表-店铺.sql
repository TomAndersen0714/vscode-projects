...
GLOBAL CROSS JOIN (
    ...
    arrayJoin(
        ...
        range(toUInt32(toDate('{{day.start}}')), toUInt32(toDate('{{day.end}}') + 1), 1))
    ) AS day
) AS day
...
SELECT
    ...
    COUNT(snick) AS snick_cnt, -- 子账号数量
...
FROM (
    -- 按天/BG/BU/店铺统计
    SELECT
        day, bg_name, bu_name, shop_id, shop_name,
        dialog_cnt, -- 会话总量
        snick_cnt, -- 子账号数量
        alert_cnt, -- 告警总量
        alert_dialog_cnt, -- 告警会话量
    ...
    -- 按天汇总
    SELECT
        day, '~' AS bg_name, '~' AS bu_name, '~' AS shop_id, '~' AS shop_name,
    ...
    FROM (
        SELECT
            ...
            SUM(level_3_finished_alert_cnt) AS level_3_finished_alert_cnt_sum,
            SUM(level_2_finished_alert_cnt) AS level_2_finished_alert_cnt_sum,
            SUM(level_1_finished_alert_cnt) AS level_1_finished_alert_cnt_sum,
