...
FROM (
    SELECT
        ...
        mark_score,
        mark_score_add,
        arraySum(rule_stats_score) + negate(arraySum(arrayFilter(x->x<0, xrule_stats_score))) AS rule_score,
        arraySum(rule_add_stats_score) + arraySum(arrayFilter(x->x>0, xrule_stats_score)) AS rule_score_add,
