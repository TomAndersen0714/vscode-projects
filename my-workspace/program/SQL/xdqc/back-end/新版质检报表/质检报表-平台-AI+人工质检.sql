...
SELECT
    CASE
        ...
        ELSE platform
    END AS `平台`,
    platform_dialog_stat_ai_check_info.*,
    platform_human_check_info.tag_name_arr AS `人工质检标签`,
    platform_human_check_info.tag_cnt_arr AS `人工质检触发次数`
FROM (
    SELECT
        platform,
    ...
    GROUP BY platform
) AS platform_dialog_stat_ai_check_info
-- 人工质检结果
GLOBAL FULL OUTER JOIN (
    SELECT
        platform,
