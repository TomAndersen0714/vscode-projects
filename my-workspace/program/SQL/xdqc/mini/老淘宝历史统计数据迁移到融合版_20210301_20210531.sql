...
-- 时间范围
WHERE toYYYYMMDD(date) BETWEEN 20210301 AND 20210531
...
"select * from ods.qc_session_count_all WHERE toYYYYMMDD(date) BETWEEN 20210301 AND 20210531 FORMAT Avro" \
> /tmp/ods.qc_session_count_all_20210301_20210531.Avro

docker exec -i 9e clickhouse-client --port=19000 --query=\
"select * from ods.qc_read_mark_detail_all WHERE toYYYYMMDD(date) BETWEEN 20210301 AND 20210531 FORMAT Avro" \
> /tmp/ods.qc_read_mark_detail_all_20210301_20210531.Avro

docker exec -i 9e clickhouse-client --port=19000 --query=\
"select * from ods.qc_question_detail_all WHERE toYYYYMMDD(date) BETWEEN 20210301 AND 20210531 FORMAT Avro" \
> /tmp/ods.qc_question_detail_all_20210301_20210531.Avro

docker exec -i 9e clickhouse-client --port=19000 --query=\
"select * from ods.qc_words_detail_all WHERE toYYYYMMDD(date) BETWEEN 20210301 AND 20210531 FORMAT Avro" \
> /tmp/ods.qc_words_detail_all_20210301_20210531.Avro


-- 老淘宝 zjk-bigdata005 复制 zjk-bigdata006 数据到OSS盘
scp root@zjk-bigdata006:/tmp/ods.qc_session_count_all_20210301_20210531.Avro /opt/bigdata/
scp root@zjk-bigdata006:/tmp/ods.qc_read_mark_detail_all_20210301_20210531.Avro /opt/bigdata/
scp root@zjk-bigdata006:/tmp/ods.qc_question_detail_all_20210301_20210531.Avro /opt/bigdata/
scp root@zjk-bigdata006:/tmp/ods.qc_words_detail_all_20210301_20210531.Avro /opt/bigdata/

-- v1mini-bigdata-002 CH(1b73e87dc75e)创建本地表, 用于备份数据!!!(上次迁移时已备份)
...
< /opt/bigdata/ods.qc_session_count_all_20210301_20210531.Avro

-- ods.qc_read_mark_detail_all
docker exec -i 1b7 clickhouse-client --port=19000 --query=\
"INSERT INTO buffer.ods_qc_read_mark_detail_buffer FORMAT Avro" \
< /opt/bigdata/ods.qc_read_mark_detail_all_20210301_20210531.Avro

-- ods.qc_question_detail_all
docker exec -i 1b7 clickhouse-client --port=19000 --query=\
"INSERT INTO buffer.qc_question_detail_buffer FORMAT Avro" \
< /opt/bigdata/ods.qc_question_detail_all_20210301_20210531.Avro

-- ods.qc_words_detail_all
docker exec -i 1b7 clickhouse-client --port=19000 --query=\
"INSERT INTO buffer.qc_words_detail_all_buffer FORMAT Avro" \
< /opt/bigdata/ods.qc_words_detail_all_20210301_20210531.Avro
