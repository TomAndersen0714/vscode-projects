-- add partition
alter table app_mp.reception_subnick_hybd_stat add if not exists range partition ${var:ds_nodash} <= VALUES < ${var:tomorrow_ds_nodash};
--
...
WHERE day = ${var:ds_nodash} and mode = 'HYBRID'
GROUP BY 1
        ,2
        ,3;
...
with send_table as (
        ...
        from dwd.mini_xdrs_log where day = ${var:ds_nodash} and mode = 'HYBRID' and act = 'send_msg' group by 1,2,3
), recv_table as (
        select snick, cnick
        from dwd.mini_xdrs_log where day = ${var:ds_nodash} and mode = 'HYBRID' and act = 'recv_msg' group by 1,2
), only_send_pair as (
        ...
        select shop_id, snick, cnick
        from dwd.mini_xdrs_log where day = ${var:ds_nodash} and send_msg_from = '4'
...
select ${var:ds_nodash}, r.shop_id, r.subnick, r.recp_buyers_amount + cnick_amount
from app_mp.reception_subnick_hybd_stat as r join extra_recp_cnicks as e using(subnick)
where r.ds_nodash = ${var:ds_nodash};
