...
FROM (
    ...
    FROM (
        ...
        WHERE day BETWEEN toYYYYMMDD(
                parseDateTimeBestEffort('{{ day.start=yesterday }}')
            ) AND toYYYYMMDD(
                parseDateTimeBestEffort('{{ day.end=yesterday }}')
            )
            AND shop_id = '{{ shop_id=61839cb5f393ab0018592ca3 }}'
            AND question_id GLOBAL IN (
                SELECT _id AS question_id
    ...
    ON question_status.question_id = sq._id
) AS b 
GLOBAL LEFT JOIN dim.subcategory_all 
ON b.subcategory_id = dim.subcategory_all._id
ORDER BY `咨询量` DESC