...
-- DROP TABLE sxx_dwd.outbound_workorder_local ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE sxx_dwd.outbound_workorder_local ON CLUSTER cluster_3s_2r
(
    `day` Int32,
    `platform` String,
    `shop_id` String,
...
-- DROP TABLE sxx_dwd.outbound_workorder_all ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE sxx_dwd.outbound_workorder_all ON CLUSTER cluster_3s_2r
AS sxx_dwd.outbound_workorder_local
ENGINE = Distributed('cluster_3s_2r', 'sxx_dwd', 'outbound_workorder_local', rand())

-- DROP TABLE buffer.sxx_dwd_outbound_workorder_buffer ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE buffer.sxx_dwd_outbound_workorder_buffer ON CLUSTER cluster_3s_2r
AS sxx_dwd.outbound_workorder_all
ENGINE = Buffer('sxx_dwd', 'outbound_workorder_all', 16, 15, 35, 81920, 409600, 16777216, 67108864)