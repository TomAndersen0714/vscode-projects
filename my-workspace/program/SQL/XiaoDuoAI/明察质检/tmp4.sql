...
ALTER TABLE sxx_tmp.outbound_workorder_local ON CLUSTER cluster_3s_2r DELETE WHERE toYYYYMM(toDateTime64(delivery_time, 3)) = 202201
...
ALTER TABLE sxx_ods.outbound_workorder_local ON CLUSTER cluster_3s_2r DROP PARTITION 20220601
ALTER TABLE sxx_ods.outbound_workorder_local ON CLUSTER cluster_3s_2r DROP PARTITION 20220722
ALTER TABLE sxx_ods.outbound_workorder_local ON CLUSTER cluster_3s_2r DROP PARTITION 20220721

ALTER TABLE sxx_ods.outbound_workorder_local ON CLUSTER cluster_3s_2r DELETE WHERE toUInt32(day/100) = 202201
...
FROM (
    ...
    WHERE toYYYYMM(toDateTime64(delivery_time, 3)) = 202202
) AS outbound_info
GLOBAL LEFT JOIN (
    SELECT DISTINCT
        platform,
