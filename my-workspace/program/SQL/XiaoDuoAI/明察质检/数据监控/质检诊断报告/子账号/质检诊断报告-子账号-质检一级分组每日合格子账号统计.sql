...
SELECT
    day,
    snick_sum,
    (snick_sum - subtract_score_snick_sum) AS qualified_snick_sum,
    snick_sum AS `质检子账号总量`,
    qualified_snick_sum AS `合格子账号总量`,
    if(qualified_snick_sum!=0, round(qualified_snick_sum/snick_sum*100, 4), 0.00) AS `子账号合格率`
FROM (
    SELECT
        day,
