-- 客户评价满意度(二期)-统计-评价转化每日统计
SELECT
    day,
    -- 全部评价记录来源统计
    SUM(1) AS `全部记录数`,
    SUM(latest_eval_time != '') AS `评价次数`,
    SUM(latest_eval_time != '' AND latest_eval_code IN [0, 1]) AS `满意次数`,
    SUM(latest_eval_time != '' AND eval_change_model = 'unstatisfy_to_statisfy') AS `挽回次数`
...
FROM (
    ...
    GROUP BY  day, dialog_id, user_nick, eval_recer, source, send_time
    -- 下拉框-最新评价等级
    HAVING (
        '{{ latest_eval_codes }}'=''
        OR
