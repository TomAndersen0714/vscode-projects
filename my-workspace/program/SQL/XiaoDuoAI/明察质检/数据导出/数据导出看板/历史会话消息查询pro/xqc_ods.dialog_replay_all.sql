-- DROP TABLE xqc_ods.dialog_replay_local ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE xqc_ods.dialog_replay_local ON CLUSTER cluster_3s_2r
(
    `_id` String,
    `platform` String,
    `channel` String,
...
PARTITION BY day
PRIMARY KEY (platform, seller_nick, begin_time)
ORDER BY (platform, seller_nick, begin_time)
SETTINGS index_granularity = 8192, storage_policy = 'rr'


-- DROP TABLE xqc_ods.dialog_replay_all ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE xqc_ods.dialog_replay_all ON CLUSTER cluster_3s_2r
AS xqc_ods.dialog_replay_local
ENGINE = Distributed('cluster_3s_2r', 'xqc_ods', 'dialog_replay_local', rand())

-- DROP TABLE buffer.xqc_ods_dialog_replay_buffer ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE buffer.xqc_ods_dialog_replay_buffer ON CLUSTER cluster_3s_2r
AS xqc_ods.dialog_replay_all
ENGINE = Buffer('xqc_ods', 'dialog_replay_all', 16, 15, 35, 81920, 409600, 16777216, 67108864)