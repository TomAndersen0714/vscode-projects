...
from (
        ...
        where platform = '%s'
            ...
            and company_id = '%s'
            and date >= %d
            and date <= %d -- (sess.Platform, req.ShopName, sess.CompanyId.Hex(), req.StartDate, req.EndDate)
        group by company_id,
            seller_nick,
            platform,
            department_id,
    ...
    left join (
        ...
        group by company_id,
            ...
            employee_name,
            employee_id
    ) as score using(company_id, department_id, employee_id)
...
from (
        ...
        where platform = '%s'
            ...
            and company_id = '%s'
            and date >= %d
            and date <= %d -- (sess.Platform, service.BuildSqlArrayById(departmentIds), req.ShopName, sess.CompanyId.Hex(), req.StartDate, req.EndDate)
        group by company_id,
            seller_nick,
            platform,
            department_id,
