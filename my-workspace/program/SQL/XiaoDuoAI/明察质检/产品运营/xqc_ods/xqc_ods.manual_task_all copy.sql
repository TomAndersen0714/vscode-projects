...
-- DROP TABLE tmp.web_log_local ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE tmp.web_log_local ON CLUSTER cluster_3s_2r
(
    `app_id` String,
    `distinct_id` String,
    `track_id` String,
...
-- DROP TABLE tmp.web_log_all ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE tmp.web_log_all ON CLUSTER cluster_3s_2r
AS tmp.web_log_local
ENGINE = Distributed('cluster_3s_2r', 'tmp', 'web_log_local', rand())

-- DROP TABLE buffer.tmp_web_log_buffer ON CLUSTER cluster_3s_2r NO DELAY
CREATE TABLE buffer.tmp_web_log_buffer ON CLUSTER cluster_3s_2r
AS tmp.web_log_all
ENGINE = Buffer('tmp', 'web_log_all', 16, 15, 35, 81920, 409600, 16777216, 67108864)
