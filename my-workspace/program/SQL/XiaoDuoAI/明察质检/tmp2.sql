...
--                  FROM cdp_ods.tag_snapshot_all))t2 ON t1.shop_id=t2.shop_id
...
--          GROUP BY t1.snick
--          ORDER BY t1.snick)t2 USING (snick)
--       ORDER BY snick)t1
--    LEFT JOIN
--      (SELECT t1.snick AS snick,
...
--          WHERE `day`=
--              (SELECT max(`day`) AS `day`
--               FROM cdp_ods.tag_snapshot_all))t2 ON t1.cnick=t2.cnick
...
--       ORDER BY t1.snick,
--                t3.group_name,
--                t3.tag_name)t2 ON t1.snick=t2.snick
--    LEFT JOIN
--      (SELECT snick,
  ...
  (SELECT t1.snick,
   ...
   FROM
     (SELECT snick,
             uniqExact(cnick) AS uv
      FROM sxx_dws.snick_new_ask_detail_daily_all
      WHERE `day` >= cast(replace('20221007', '-', '') AS int)
        AND `day` <= cast(replace('20221009', '-', '') AS int)
        AND shop_id = '{{ shop_id_var=6143f37218f6b6000e173bc3 }}'
      GROUP BY snick
      ORDER BY snick)t1
   INNER JOIN
      ...
      JOIN
        (SELECT `day`,
         ...
         WHERE tag_id GLOBAL IN
           ...
           AND `day`=
             (SELECT max(`day`) AS `day`
              FROM cdp_ods.tag_snapshot_all) AND shop_id = '{{ shop_id_var=6143f37218f6b6000e173bc3 }}')t2 ON t1.shop_id=t2.shop_id
      AND t1.cnick=t2.cnick
      GROUP BY t1.snick
      ORDER BY t1.snick)t2 USING (snick)
   ORDER BY snick)t1
  ...
  (SELECT t1.snick AS snick,
   ...
   JOIN
     (SELECT `day`,
      ...
      WHERE `day`=
          (SELECT max(`day`) AS `day`
           FROM cdp_ods.tag_snapshot_all) AND shop_id = '{{ shop_id_var=6143f37218f6b6000e173bc3 }}')t2 ON t1.cnick=t2.cnick
   AND t1.shop_id =t2.shop_id
   JOIN
     (SELECT tag_id,
             group_name,
             tag_name,
             shop_id
      FROM cdp_dim.wt_tag_all WHERE shop_id = '{{ shop_id_var=6143f37218f6b6000e173bc3 }}')t3 ON t2.tag_id=t3.tag_id
   AND t2.shop_id=t3.shop_id
   GROUP BY t1.snick,
            t3.group_name,
            t3.tag_name
...
INNER JOIN
  ...
  FROM 
   ...
   WHERE shop_id = '{{ shop_id_var=6143f37218f6b6000e173bc3 }}'
   GROUP BY subnick)
   WHERE name='售前全员')t4 ON t1.snick=t4.snick
ORDER BY t1.snick,
         t2.group_name,
         t2.tag_name)
GROUP BY "店铺id",
