-- 数据大屏二级监控
集团组织架构表(xqc_dim.company_department_all)
...
GLOBAL LEFT JOIN (
    ...
    GLOBAL LEFT JOIN (
        SELECT  -- 子账号维度当天告警数据聚合统计
            ...
            sum(`level` = 2 AND is_finished = "True") AS shop_today_level_2_finished_cnt, -- 子账号当天中级已处理告警量
            sum(`level` = 3 AND is_finished = "True") AS shop_today_level_3_finished_cnt, -- 子账号当天高级已处理告警量
...
LEFT JOIN ( -- BG, BU, shop_name, snick, superior_name, employee_name, cnick, dialog_id
    SELECT * 
    FROM ( -- BG,BU,shop_name,snick
        SELECT *
        FROM (
            SELECT
