...
-- 告警趋势图(全局告警统计)
SELECT day AS `日期`,
    tb_alert_count AS `淘宝`,
    jd_alert_count AS `京东`
FROM (
        SELECT arrayJoin(
            arrayMap(
                x->toYYYYDDMM(toDate(x)),
                range(toUInt32(toDateTime('{{day.start}}')), toUInt32(toDateTime('{{day.end}}') + 1), 86400)
            )
        ) AS day
) AS time_axis
GLOBAL LEFT JOIN (
    SELECT day,
        sum(platform = 'tb') as tb_alert_count,
        sum(platform = 'jd') as jd_alert_count
    FROM xqc_ods.event_alert_all FINAL
    WHERE day BETWEEN {{day.start}} AND {{day.end}}
        AND shop_id GLOBAL IN (
            SELECT mp_shop_id AS shop_id
            FROM xqc_dim.shop_all
