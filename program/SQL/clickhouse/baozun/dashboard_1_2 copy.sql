...
SELECT
    ...
    if(level_2_cnt!=0, round(sum(level=2 and is_finished)/level_2_cnt*100,2), 0.00), -- 中级告警完结率
    if(level_3_cnt!=0, round(sum(level=3 and is_finished)/level_3_cnt*100,2), 0.00) -- 高级告警完结率
FROM
