...
select {{ ds_nodash }}, shop_name, sum(if(act='recv_msg', 1, 0)), sum(is_identified), count(distinct if(act='send_msg' and send_msg_from='0', msg_id, null)) from dwd.mini_xdrs_log where day = {{ ds_nodash }} group by 1,2;

with dim_shop as (
        select shop_id, shop_name from dwd.mini_xdrs_log where day={{ ds_nodash }} group by 1,2
)
...
from app_mp.operation_shop_stat where ds_nodash = {{ ds_nodash }};