...
t2 AS (
  SELECT t1.*,
    ...
    subcategory_id
  FROM t1
    LEFT JOIN ods.question_b ON t1.question_id = ods.question_b._id
),
t3 AS (
  SELECT t2.*,
    name
...
SELECT "{{ params.date_no_dash }}" AS stat_day,
  platform,
  snick,
  snick_oid,
  plat_goods_id,
